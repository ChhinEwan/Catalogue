...
def afficher_liste_livres(livres, titre):
    ...
    for numero, livre in enumerate(livres, start=1):
        ...
        print(f"   Auteur : {livre.auteur} | Éditeur : {livre.editeur or 'Non répertorié'} | Date : {livre.date_parution or 'Date inconnu'} | Genre : {livre.genre or 'Genre non repertorie'}")
        # Description preview (tronquée à 700 caractères)
        desc_preview = getattr(livre, 'description', None) or ""
        if desc_preview:
            if len(desc_preview) > 700:
                desc_preview = desc_preview[:700].rstrip() + "..."
...
def afficher_resultats_recherche(livres):
    ...
    for numero, livre in enumerate(livres, start=1):
        ...
        print(f"   Éditeur : {livre.editeur or 'Non répertorié'} | Date : {livre.date_parution or 'Date inconnu'} | Genre : {livre.genre or 'Genre non repertorie'}")
